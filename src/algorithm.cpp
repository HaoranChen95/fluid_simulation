...
void cell_list(void) {
  ...
  int64_t cx, cy, cz;
  for (uint64_t i = 0; i < Nm; i++) {
    cx = static_cast<uint64_t>(r0[0][i] / Cell_l[0]);
    cy = static_cast<uint64_t>(r0[1][i] / Cell_l[1]);
    cz = static_cast<uint64_t>(r0[2][i] / Cell_l[2]);
...
void calc_pos(void) {
  ...
  for (i = 0; i < Nm; i++) {
    for (ax = 0; ax < 3; ax++) {
      r0[ax][i] = r1[ax][i];
      r1[ax][i] += v[ax][i] * dt + f1[ax][i] * half_dt2;
    }
  }
  // for (ax = 0; ax < 3; ax++) {
  //   std::cout<< " r " << r0[ax][0] << " v " << v[ax][0] * dt << " f "
  //             << f1[ax][0] * half_dt2;
  // }
  // std::cout << std::endl;
}
